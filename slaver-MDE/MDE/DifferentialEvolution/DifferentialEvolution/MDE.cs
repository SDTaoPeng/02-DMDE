...
//-------------------- 参考Multifactor-inﬂuenced energy consumption forecasting using enhanced back-propagation neural network文献----------------

            Random rd = new Random();
            double a;
            double b;
            a = (double)Math.Round(((decimal)rd.Next(100, 200) / 200), 2);    //随机选取a,b，保证在[0.5, 1]之间
            b = (double)Math.Round(((decimal)rd.Next(100, 200) / 200), 2);

            while ((I_iter < I_itermax) && (S_bestval.FVr_oa[0] > F_thresholdmin))
            {
                //-------------- 参考An Adaptive Multipopulation Differential Evolution With Dynamic Population Reduction文献-----------
                int NPinitial = 100;
                int Msize;
                int h;                                                                //随机指数
                Msize = NPinitial / 50;
                h = (int)Math.Round(((decimal)rd.Next(1, Msize)));                    //生成随机指数h
                double Fi;
                double CRi;
                double c = (double)Math.Round(((decimal)rd.Next(0, 200) / 200), 2);   //c属于[0,1] 文献公式9-13（Fig.2 第22步）

                List<double> Sf = new List<double>();     //文献（Fig.2 第16步）
                List<double> Scr = new List<double>();    //文献（Fig.2 第16步）
                
                if (I_iter <= I_itermax / 2)
                {

                    for (int i1 = 0; i1 < NPinitial;i1++ )
                    {
                        //--------------------------------------------------获取F和CR的表达式-----------------------------------------------
                        //起始的F和CR改为：初始化最优的F和CR，与文献Multifactor-inﬂuenced energy consumption forecasting using enhanced back-propagation neural network结合--------
                        F_weight = a + (1 - a) * Math.Sin((I_iter / I_itermax) * Math.PI - 0.5 * Math.PI / 2);
                        //F_weight = a - (1 - a) * Math.Cos(Math.PI / 2 - (I_iter / I_itermax) * Math.PI);
                        F_crossoverprob = b + (1 - b) * Math.Sin((I_iter / I_itermax) * Math.PI - 0.5 * Math.PI / 2);
                        //F_crossoverprob = b - (1 - b) * Math.Cos(Math.PI / 2 - (I_iter / I_itermax) * Math.PI);

                        //基于内存的DE： 文献为：An Adaptive Multipopulation Differential Evolution With Dynamic Population Reduction

                        //生成随机的uF_weight和uF_crossoverprob，文献公式7-8（Fig.2 第9步）
                        Fi = (double)Math.Round(((decimal)rd.Next((int)(F_weight * 200), 200) / 200), 2);
                        CRi = (double)Math.Round(((decimal)rd.Next((int)(F_crossoverprob * 200), 200) / 200), 2);
                        ...
                        //---------------------添加判断条件multi-mutation，参考：--------------------------------
                        double pG;                 //概率
                        double pmin = 0; double pmax = 1;
                        pG = pmin + (pmax - pmin) * I_iter / I_itermax;
                        double rand01;
                        rand01 = (double)Math.Round(((decimal)rd.Next(0, 200) / 200), 2);    //随机选取a,b，保证在[0.5, 1]之间
                        if (I_strategy == 1)
                        {
                            for (i = 0; i < I_popnumber; i++)
                                for (j = 0; j < I_dimensionspara; j++)
                                    //加判断函数         
                                    //FM_ui[i, j] = FM_pm3[i, j] + F_weight * (FM_pm1[i, j] - FM_pm2[i, j]);         //Mutation operation
                                    if (rand01 < pG)
                                    {
                                        FM_ui[i, j] = FM_pm3[i, j] + F_weight * (FM_pm1[i, j] - FM_pm2[i, j]);         //Mutation operation
                                    }
                                    ...
                                    {
                                        FM_ui[i, j] = FM_pm5[i, j] + F_weight * (FM_pm3[i, j] - FM_pm4[i, j]) + F_weight * (FM_pm1[i, j] - FM_pm2[i, j]);         //Mutation operation
                                    }
                            for (i = 0; i < I_popnumber; i++)
                                for (j = 0; j < I_dimensionspara; j++)
                    ...
                    double meanScr;
                    double allScr=0;
                    for (int nf = 0; nf < Sf.Count;nf++ )
                    {
                        allSf =allSf+ Sf[nf];
                    }
                    meanSf = allSf / (Sf.Count);
                    F_weight = F_weight * (1 - c) + c * meanSf;

                    for (int ncr = 0; ncr < Scr.Count; ncr++)
                    {
                        allScr = allScr + Scr[ncr];
                    }
                    meanScr = allScr / (Scr.Count);
                    F_crossoverprob = F_crossoverprob * (1 - c) + c * meanScr;
